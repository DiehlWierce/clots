...
/**
 * Что делает противник. Те же три глагола, что и у игрока, — бой читается,
 * потому что обе стороны играют по одним правилам.
 *
 * Прежних намерений было семь, а приёмов у игрока пять, и они не совпадали:
 * игрок не мог понять, чем ответить на «Пробой» или «Истощение». Замер
 * показал, что и отвечать было незачем — спам обычного удара обыгрывал
 * любую тактику, потому что бой длится два-три хода.
 */
export type IntentKind = 'strike' | 'charge' | 'mend'
...
export interface IntentDef {
  ...
  threat: number
  /** Доля максимума здоровья, которую враг возвращает себе. */
  healSelf?: number
}
...
export interface EnemyDef {
  ...
  hp: number
  attack: number
  /** Плоская броня: снижает урон игрока, пробивается супер-ударом. */
  armor: number
  /** Регенерация врага за ход. */
  regen?: number
  /** Порядок намерений, повторяется по кругу — телеграфируется игроку. */
  pattern: IntentKind[]
  /** Приём, к которому враг уязвим: урон ×1.5. */
  weakness?: 'strike' | 'super'
}

/**
 * Приёмы игрока. «Замах» — подготовка супер-удара: ход без урона, но
 * входящий удар в этот ход вдвое слабее, поэтому замах никогда не бывает
 * чистой потерей. Прежний «Фокус» этим и был плох: ×1.6 за потерянный ход,
 * где ход стоит ×1.0.
 */
export type PlayerCombatAction = 'strike' | 'charge' | 'super' | 'mend'
...
export interface CombatState {
  ...
  intentIndex: number
  /** Игрок замахнулся: следующий удар — супер, входящий урон вдвое слабее. */
  charging: boolean
  /** Противник замахнулся: его следующий ход — супер-удар. Это видно заранее. */
  enemyCharging: boolean
  /** Сколько целостности игрок восстановил за этот бой. */
  mended: number
...
export interface GameState {
  ...
  /**
   * Сколько целостности уже восстановлено в этом цикле. Обнуляется при
   * завершении цикла: лечить можно не больше доли максимума за ход.
   */
  healedThisCycle: number
  /**
   * Уровень перегрузки ядра: бесконечный сток поздних ресурсов.
   * Каждый уровень дороже предыдущего и даёт небольшую постоянную прибавку.
   */
  overdrive: number
