...
export function EventOverlay({ state, eventId, dispatch }: Props) {
  const event = EVENT_BY_ID.get(eventId)
  if (!event) return null

  return (
    <div className="overlay" role="dialog" aria-modal="true" aria-label={event.title}>
      <div className="overlay__card">
        <div className="overlay__grip" aria-hidden="true" />
        <h2 className="overlay__title">❗ {event.title}</h2>
        <p className="overlay__sub">{event.text}</p>
          ...
          {event.options.map(option => {
            ...
            return (
              ...
              >
                ...
                <span className="action__desc">{option.outcome}</span>
